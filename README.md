...
npx laqrumbrain-reembed --from local-bge-m3 --dry-run

# Then run for real (resumable on interruption)
npx laqrumbrain-reembed --from local-bge-m3
```
...
Set `embedding.dimensions` to match the server's native output dim. LaqrumBrain verifies the returned vector size on every response and fails loudly if the server ignores the `dimensions` parameter — preventing silently corrupted indexes.
...
LaqrumBrain is aggressively optimized for token efficiency and latency, informed by analysis of the Claude Code source.
