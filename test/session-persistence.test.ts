...
describe("writeHandoffFileSync", () => {
  ...
  it("writes a valid JSON file", () => {
    ...
    const path = join(dir, ".laqrumbrain-handoff.json");
...
describe("readAndDeleteHandoffFile", () => {
  ...
  it("reads and deletes the handoff file", () => {
    ...
    expect(existsSync(join(dir, ".laqrumbrain-handoff.json"))).toBe(false);
    expect(existsSync(join(dir, ".laqrumbrain-handoff.json.processing"))).toBe(false);
  });
  ...
  it("truncates long fields for safety", () => {
    dir = makeTmpDir();
    writeFileSync(join(dir, ".laqrumbrain-handoff.json"), JSON.stringify({
  ...
  it("rejects prototype pollution attempts", () => {
    ...
    writeFileSync(join(dir, ".laqrumbrain-handoff.json"),
  ...
  it("handles corrupted JSON gracefully", () => {
    dir = makeTmpDir();
    writeFileSync(join(dir, ".laqrumbrain-handoff.json"), "not json{{{");
    const result = readAndDeleteHandoffFile(dir);
    expect(result).toBeNull();
  });
  ...
  it("handles non-object JSON (array)", () => {
    dir = makeTmpDir();
    writeFileSync(join(dir, ".laqrumbrain-handoff.json"), "[1,2,3]");
    const result = readAndDeleteHandoffFile(dir);
    expect(result).toBeNull();
  });
  ...
  it("cleans up stale .processing files", () => {
    ...
    writeFileSync(join(dir, ".laqrumbrain-handoff.json.processing"), "stale data");
